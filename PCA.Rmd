...
When looking at the scre eplot, it is important to look for elbows, which means a point after which the eigenvalues start decrease more slowly. As we can see from the plot, the elbow appears passing from the second to the third dimension.
...
The red dashed line on the graph above indicates the expected average contribution. For a given component, a variable with a contribution larger than this cutoff could be considered as important in contributing to the component.
...
As we can observe the data are quite spread along each directions. The most significant cluster that can be observed is the one representing the cars of the Minivan type in the bottom-right corner.

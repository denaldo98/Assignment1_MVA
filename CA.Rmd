...
### Let's perform CA on the first 8 columns
```{r}
...
The function fviz_ca_biplot() of the package factoextra can be used to draw the biplot of rows and columns variables.
```{r}
...
```{r}
head(res.ca$row$contrib)  # how much each row contributes to each dimension
```
...
fviz_ca_col(res.ca, repel = TRUE, col.col ="steelblue", shape.col = 15) # change shape and color of the row points
```
The plot above shows the relationships between row points:
...
##### Quality of representation of column
...
```{r}
head(res.ca$col$cos2,3) # quality of representation of first 3 col points
```
The values of the cos2 are comprised between 0 and 1. The sum of the cos2 for rows on all the CA dimensions is equal to one.
...
```{r}
res.ca$col$coord  # coordinates of the cols (i.e. age ranges) in the new dimensions
```

```{r}
res.ca$col$contrib  # how much each col contributes to each dimension
```

```{r}
res.ca$col$cos2 # quality of representaitio
```

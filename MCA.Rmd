...
As we are not using all of them as active variables, we will create a new dataframe only with relevant columns:
```{r}
...
bicing_mca = MCA(X2, ncp=10, quanti.sup=7:8, level.ventil=0.01)
```
## Questions
...
bicing_mca$eig
```
After looking at the MCA results above, we can see that this MCA generates 16 dimensions and not 10 as we requested. The fact that 16 dimensions are present means that there was 16 distinct eigenvalues during the decomposition and thus 16 dimensions (which together explain 100% of the variance of the data).
